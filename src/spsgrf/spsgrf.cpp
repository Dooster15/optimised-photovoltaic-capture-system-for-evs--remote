// =================
//  File Name: spsgrf.cpp
//  Author: Jack Holdsworth
//  ID: 20023933
//  Create Time: 2024-01-26 13:06:46
  //  Modified time: 2024-04-24 12:06:44
//  Description: spsgrf setup and function commands
// =================
 
#include "spsgrf.hpp"
// setup and commands provided by provided by SPIRIT1 DK
Spirit::Spirit() {}

void Spirit::SpiritSetup()
{
...
uint8_t* Spirit::SpiritGet()
{
	SpiritPutInRX();
	
	// wait until take in receive buffer
	while (SpiritRead(124, rxBuffer) == 0)
	{
		delay(100);
	}

	// print rxbuffer to serial monitor
	for (int i = 0; i <= 124; i++)
	{
...
void Spirit::SpiritSpiCommandStrobes(uint8_t cmd_code)
{
...
void Spirit::SpiritSpiReadRegisters(uint8_t address, uint8_t n_regs, uint8_t *buffer)
{
...
void Spirit::SpiritSpiWriteRegisters(uint8_t address, uint8_t n_regs, uint8_t *buffer)
{
	uint8_t temp = 0;
...
uint8_t Spirit::SpiritRead(uint8_t n_regs, uint8_t *buffer)
{
	uint8_t stateC0;
	uint8_t stateC1;
	uint8_t tmp;
	...
	if ((stateC1 & 0xFE) == 0x66)
		return 0; // still in RX

	for (i = 0; i < n_regs; i++)
	{
		SpiritSpiReadRegisters(0xC0, 1, &stateC0);
		if ((stateC0 & 0x02) == 0x00)
		{ // if fifo not empty
...
void Spirit::SpiritSend(uint8_t n_regs, uint8_t *buffer)
{
	uint8_t state;
...
void Spirit::SpiritVcoCalibration(void)
{
	uint8_t tmp[4];
	uint8_t cal_words[2];
	uint8_t state;
...
void Spirit::SpiritBaseConfiguration(void)
{
	uint8_t tmp[7];
	...
	*/
	...
	tmp[0] = 0x00; /* reg. PCKT_FLT_OPTIONS (0x4F) */
